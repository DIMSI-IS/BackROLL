...
def borg_rc(cmd):
  ...
  if cmd.returncode != 0:
    ...
    error_msg = error_msg.replace("Warning: Attempting to access a previously unknown unencrypted repository!\nDo you want to continue? [yN] yes (from BORG_UNKNOWN_UNENCRYPTED_REPO_ACCESS_IS_OK)", "")
    error_msg = error_msg.strip("\n")
    raise ValueError(error_msg)
  if not cmd.stdout: return None
  return json.loads(cmd.stdout.strip("\n"))
    

def get_backup(virtual_machine, backup_name):
  storage_repository = storage.retrieveStoragePathFromHostBackupPolicy(virtual_machine)
  borg_repository = f"{storage_repository['path']}{virtual_machine['name']}"
  request = subprocess.run(["borg", "info", "--json", f"{borg_repository}::{backup_name}"], text=True, capture_output=True)
...
def delete_backup(virtual_machine, backup_name):
  ...
  request = subprocess.run(["borg", "delete", f"{borg_repository}::{backup_name}"], text=True, capture_output=True)
  return borg_rc(request)
