...
def read_external_hook(identity: Json = Depends(auth.valid_token)):
...
def update_external_hook(hook_id, item: items_create_external_hook, identity: Json = Depends(auth.valid_token)):
  try:
...
def delete_external_hook(hook_id, identity: Json = Depends(auth.valid_token)):
  ...
  if not hook_id: raise HTTPException(status_code=404, detail='Pool not found')
