      ...
      <div class="row" v-if="$store.state.isvmTableReady">
        ...
        >
          <va-card class="mb-4" :color="info.light">
            <va-card-content>
              <p class="va-h2 mb-0">{{ info.value }}</p>
          ...
          <va-card>
            ...
            <va-card-content>
              <div class="row row-separated">
                <div class="flex xs6">
                  <p class="va-h2 mb-1 va-text-center" :style="{color: theme.success}">{{ successTaskNumber }}</p>
                  <p class="va-text-center mb-1">{{$t('dashboard.info.success')}}</p>
                </div>
                <div class="flex xs6">
                  <p class="va-h2 mb-1 va-text-center" :style="{color: theme.danger}">{{ failureTaskNumber }}</p>
                  <p class="va-text-center no-wrap mb-1">{{$t('dashboard.info.failure')}}</p>
                </div>
              </div>
            </va-card-content>
          </va-card>
          ...
          <va-list>
            ...
            >
              ...
              <va-list-item-section icon>
                <va-chip size="small" outline square :color="(100 - ((storage.info.free / storage.info.total) * 100)).toFixed(1) < 75 ? 'success' : 'danger'">
                  <va-icon v-if="(100 - ((storage.info.free / storage.info.total) * 100)).toFixed(1) >= 75" name="warning" color="danger" />
                  <span style="margin-right: 2px;"><b>{{ (100 - ((storage.info.free / storage.info.total) * 100)).toFixed(1) }}% Used</b></span> ({{ humanStorageSize(storage.info.free) }} free)
                </va-chip>
              </va-list-item-section>
            </va-list-item>
          </va-list>
