...
export default {
  ...
  methods: {
    addStorage() {
      const self = this
      axios.post(`${this.$store.state.endpoint.api}/api/v1/storage`, { name: this.inputValue1, path: this.inputValue2 }, { headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ${this.$keycloak.token}`}})
      .then(response => {
