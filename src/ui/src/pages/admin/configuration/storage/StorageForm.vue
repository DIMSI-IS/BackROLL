  ...
  <va-card>
    ...
    <va-card-content v-if="!storageId || stateStorage">
      <va-form tag="form" @submit.prevent="storageId ? updateStorage() : addStorage()">
        <va-input label="Name" v-model="formStorage.name"
...
export default {
  ...
  computed: {
    stateStorage() {
      return this.$store.state.storageList.find((item) => {
  ...
  methods: {
    updateStorage() {
      ...
      this.$store.dispatch("updateStorage", { vm: this, token: this.$keycloak.token, storageId: id, name, path })
    },
    addStorage() {
      const self = this
      const { name, path } = this.formStorage
      axios.post(`${this.$store.state.endpoint.api}/api/v1/storage`, { name, path }, { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.$keycloak.token}` } })
        .then(response => {
