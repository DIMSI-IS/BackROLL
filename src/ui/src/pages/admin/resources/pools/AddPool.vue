...
export default {
  ...
  methods: {
    ...
    addPool() {
      axios.post(`${this.$store.state.endpoint.api}/api/v1/pools`, { name: this.inputValue1, policy_id: this.policySelection.value }, { headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ${this.$keycloak.token}`}})
      .then(response => {
